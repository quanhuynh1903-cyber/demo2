...
def show():
    st.header("📝 Input Product Review")
    
    # 1. Nạp mô hình (Đảm bảo file .pkl nằm trong thư mục models)
    model_path = os.path.join('models', 'sentiment_model.pkl')
    
    if os.path.exists(model_path):
        model = joblib.load(model_path)
    else:
        st.error("Chưa tìm thấy file sentiment_model.pkl trong thư mục models!")
        return

    # 2. Giao diện nhập liệu
    user_input = st.text_area("Enter a product review (Vietnamese or English):", placeholder="Ví dụ: Sản phẩm rất tốt...")
